...
import { addressDetail, updateAddresses } from "../../lib/api/AddressApi"
...
export default function AddressEdit() {
    ...
    async function handleSubmit(e) {
        e.preventDefault()
        const response = await updateAddresses(token, id, {addressId, street, city, province, country, postal_code})
        const responseBody = await response.json()
        console.log(responseBody)

        if(response.status === 200){
            await alertSuccess("Success Edit")
        }else{
    ...
    useEffectOnce(()=>{
        fetchContact()  
            .then(()=>console.log("contact fetch success"))
        fetchAddress()
            .then(()=>console.log("addresses fetch success"))
    })
    return <>
        <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="text" id="city" name="city" className="w-full pl-10 pr-3 py-3 bg-gray-700 bg-opacity-50 border border-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" placeholder="Enter city" defaultValue="New York" value={city} onChange={(e)=>setCity(e.target.value)} required />
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="text" id="province" name="province" className="w-full pl-10 pr-3 py-3 bg-gray-700 bg-opacity-50 border border-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" placeholder="Enter province or state" defaultValue="NY" value={province} onChange={(e) => setProvince(e.target.value)} required />
                                </div>
                            </div>
                        </div>
