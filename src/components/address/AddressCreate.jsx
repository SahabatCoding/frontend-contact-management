...
import { createAddresses } from "../../lib/api/AddressApi";
...
export default function AddressCreate() {  
    ...
    async function handleSubmit(e){
        ...
        if(response.status === 200){
            await alertSuccess("Berhasil menambah data addresses")
        }else{
    ...
    useEffectOnce(()=>{
        fetchContact()
            .then(()=>console.log("Contact Success"))
    })

    return <>
        <div>
            <div className="flex items-center mb-6">
                <Link to={`/dashboard/contacts/${contact.id}`} className="text-blue-400 hover:text-blue-300 mr-4 flex items-center transition-colors duration-200">
                ...
                <div className="p-8">
                    ...
                    <form onSubmit={handleSubmit} >
                        ...
                        <div className="flex justify-end space-x-4">
                            <Link to={`/dashboard/contacts/${contact.id}`} className="px-5 py-3 bg-gray-700 text-white rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 flex items-center shadow-md">
